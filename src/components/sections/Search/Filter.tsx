...
const SearchFilter: React.FC<SearchFilterProps> = ({ isLoading, onSearchSubmit, ...props }) => {
  ...
  const enableFetch = debouncedSearchQuery.trim().length >= 2 && !isLoading;
  ...
  }, [debouncedSearchQuery, onSearchSubmit]);

  const handleSubmit = useCallback(
    (e: React.FormEvent) => {
      e.preventDefault();
      const trimmed = searchQuery.trim();
      setTriggered(!isEmpty(trimmed));
      onSearchSubmit?.(trimmed);
      if (trimmed && !searchHistories.includes(trimmed)) {
        const newHistories = [...searchHistories, trimmed];
    ...
    [searchQuery, searchHistories, onSearchSubmit],
  );
  ...
  return (
    <form
      ...
      className={cn("flex w-full max-w-2xl flex-col justify-center gap-5 text-center pt-2 md:pt-6 px-4")}
      {...props}
    >
      ...
      <div className="relative flex flex-col gap-2">
        <div className="flex items-center justify-center gap-2">
          <SearchInput
            autoFocus
            placeholder={`Search ${content === "movie" ? "movies" : "TV shows"} by title, actor, or genre...`}
            isLoading={isLoading}
            value={searchQuery}
            onValueChange={(val) => {
              setSearchQuery(val);
              if (isEmpty(val.trim())) setTriggered(false);
            }}
